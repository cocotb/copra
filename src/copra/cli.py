...
cocotb test functions.
    ...
    def _execute_command(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if getattr(args, 'module', None):
                print(f"[copra] Make sure the module '{args.module}' is importable", file=sys.stderr)
            return 1
    ...
    def _get_dut_handle(self, args: argparse.Namespace):
    ...
    def _get_dut_from_sources(self, args: argparse.Namespace):
    ...
    def _get_dut_from_module(self, args: argparse.Namespace):
    ...
    def _discover_hierarchy(self, dut, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    def _generate_output(self, hierarchy: Dict[str, Any], args: argparse.Namespace) -> int:
        ...
        output_file = getattr(args, 'outfile', None) or getattr(args, 'output', None) or 'output.pyi'
    ...
    def _generate_stub_file(self, hierarchy: Dict[str, Any], output_path: Path, args: argparse.Namespace) -> None:
        """Generate stub file.
    ...
    def _generate_documentation(self, hierarchy: Dict[str, Any], output_path: Path, args: argparse.Namespace) -> None:
    ...
    def _generate_json_output(self, hierarchy: Dict[str, Any], output_path: Path, args: argparse.Namespace) -> None:
    ...
    def _generate_yaml_output(self, hierarchy: Dict[str, Any], output_path: Path, args: argparse.Namespace) -> None:
    ...
    def _show_statistics(self, hierarchy: Dict[str, Any], args: argparse.Namespace) -> None:
        ...
        if hasattr(hierarchy, '_discovery_stats'):
            stats = hierarchy._discovery_stats
...
class StubGenerator:
    ...
    def generate_from_sources(self, sources: List[str], top_module: str, 
                            output_path: str, simulator: str = 'icarus') -> str:
        """Generate stubs from HDL sources.
...
def handle_validate_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Discover hierarchy
        hierarchy = cli._discover_hierarchy(dut, args)
        
        # Validate interface
        validation_result = validate_dut_interface(hierarchy, expected_interface)
        ...
        if validation_result['is_valid']:
            ...
            print(f"[copra] DUT interface validation failed: {validation_result['errors']}", file=sys.stderr)
            return 1
